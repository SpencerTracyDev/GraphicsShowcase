...
void InitializeScene(Scene &scene)
{
    ...
    try {
		...
		img = glimg::loaders::stb::LoadFromFile("earth.png");
        scene.earthDay = glimg::CreateTexture(img, 0);
		glBindTexture(GL_TEXTURE_2D, scene.earthDay);
        glGenerateMipmap(GL_TEXTURE_2D);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);

		img = glimg::loaders::stb::LoadFromFile("effects.png");
        scene.earthNight = glimg::CreateTexture(img, 0);
...
void DrawModel(Scene &scene, const int program, Model* m, mat4x4& ModelTr)
{
    ...
    loc = glGetUniformLocation(program, "earthNight");
    glUniform1i(loc, 1);
	...
	loc = glGetUniformLocation(program, "earth");
    glUniform1i(loc, 1);
...
void DrawSpheres(Scene &scene, unsigned int program, mat4x4& ModelTr)
{
    ...
    loc = glGetUniformLocation(program, "ModelMatrix");
    glUniformMatrix4fv(loc, 1, GL_FALSE, value_ptr(mat4x4(1.0)));
    loc = glGetUniformLocation(program, "NormalMatrix");
    glUniformMatrix4fv(loc, 1, GL_FALSE, value_ptr(mat4x4(1.0)));
    CHECKERROR;
}
